 ...
 *
 * Boyer-Moore BNDM from G. Navarro and M. Raffinot, Fast and Flexible
 * String Matching by Combining Bit-Parallelism and Suffix Automata,
 * ACM Journal of Experimental Algorithmics 5 (2000), Section 5.1.
 */
...
int search(unsigned char *x, int m, unsigned char *y, int n) {
  unsigned int B[SIGMA], D, bit, high_bit, all_bits;
  ...
  border = 0;
  for (i = filter_len - 1; i > 0; i--)
    if (memcmp(x, x + filter_len - i, (size_t)i) == 0) {
      border = i;
      break;
    }
  ...
  while (j <= n - m) {
    ...
    if (D == 0) {
      ...
      /* Align the factor with its rightmost occurrence in the pattern. */
